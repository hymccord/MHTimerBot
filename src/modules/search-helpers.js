...
function getSearchedEntity(input, values) {
    ...
    const matches = values
        .map((v) => {
            const [, score] = fuzzyMatch(input, v.value);
            return {
                entity: v,
                score,
            };
...
// Essentially Sublime Text's fuzzy match
...
function fuzzyMatchRecursive(pattern, str, patternCurIndex, strCurrIndex, srcMatces, matches, maxMatches, nextMatch, recursionCount, recursionLimit) {
    ...
    while (patternCurIndex < pattern.length && strCurrIndex < str.length) {
        ...
        if (pattern[patternCurIndex].toLowerCase() === str[strCurrIndex].toLowerCase()) {
            ...
            const [matched, recursiveScore] = fuzzyMatchRecursive(pattern, str, patternCurIndex, strCurrIndex + 1, matches, recursiveMatches, maxMatches, nextMatch, recursionCount, recursionLimit);
            if (matched) {
